...
import { Employee, Incident, PpeItem, Training, Waste, WasteLog, Inspection } from '../types';
...
const Dashboard: React.FC = () => {
    ...
    // State for date filters for inspection charts
    ...
    const filteredInspectionsWithViolation = useMemo(() => {
        return inspections.filter(inspection => {
            ...
            if (startDate) {
                ...
                if (inspectionDate < filterStartDate) return false;
            }

            if (endDate) {
                const filterEndDate = new Date(endDate + 'T23:59:59');
                if (inspectionDate > filterEndDate) return false;
            }
    ...
    const violationsByArea = useMemo(() => {
        const data = filteredInspectionsWithViolation.reduce((acc, inspection) => {
            ...
            acc[area] = (acc[area] || 0) + 1; // Count one violation per inspection record with violation = true
            return acc;
